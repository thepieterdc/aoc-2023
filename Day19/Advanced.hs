...
import Day19.Common (Condition (GreaterThan, LessThan), Rating, WorkflowStep (..), isAccepted, parse, score, solve)
...
run :: Map String [WorkflowStep] -> Int
run wfs = accepted
  where
    splits = foldl findSplits (Map.fromList $ map (,Set.fromList [0, 4000]) "xmas") $ concat $ Map.elems wfs
