...
splitGears gears parts (((row, startCol), (_, endCol), Part num) : rest) = splitGears gears (parts ++ [(Set.fromList [(row, c) | c <- [startCol..endCol]], read num :: Int)]) rest
splitGears gears parts [] = (Set.elems gears, parts)

usedGears :: [Coordinate] -> [(Set Coordinate, Int)] -> [Int]
usedGears (gear:gears) parts = usedGear gear : usedGears gears parts where
    usedGear :: Coordinate -> Int
    usedGear g = if length nbParts == 2 then product nbParts else 0 where
