...
tilt :: Grid -> Grid
tilt grid = transpose $ tilt' (transpose grid)
  where
    tilt' :: Grid -> Grid
    tilt' (row : rows) = intercalate [] (map (sortOn Data.Ord.Down) $ split (keepDelimsL $ oneOf [Just Cube]) row) : tilt' rows
    tilt' [] = []

main :: IO ()
main = loadInput >>= print . score . tilt . parse
